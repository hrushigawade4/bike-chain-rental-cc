...
contract BikeChain {
   ...
   modifier isRenter(address walletAddress) {
        require(msg.sender == walletAddress, "You can only managae your account");
        _;
    } 
    ...
    function getTotalDuartion(address walletAddress) public isRenter(walletAddress) view returns (uint) {
    ...
    // withdraw owner profit balance through renting the bike
    ...
    function setDue(address walletAddress) internal {
        uint timespanInMinutes = getTotalDuartion(walletAddress);
    ...
    function makePayment(address walletAddress,uint amount) public isRenter(walletAddress) {
        ...
        require(renters[walletAddress].balance > amount, "You do not have enough funds to cover payment. pls make a deposit.");
