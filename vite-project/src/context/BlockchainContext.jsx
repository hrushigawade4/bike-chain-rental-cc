...
export const BlockchainProvider = ({ children }) => {
    ...
    const getTotalDuartion = async() => {
        try{
            if (currentAccount) {
                const totalDuration = await contract.getTotalDuartion(currentAccount)
    ...
    const makePayment = async(value) => {
        try {
            ...
            await getRenter()
            await getRenterBalance()
            await getTotalDuartion()
            await getDue();
            
        } catch (error) {
            //another method toast error(error.data.message), but it throws error
            toast.error("execution reverted: You do not have anything due at this time or You do not have enough funds to cover payment. pls make a deposit.", {
    ...
    const checkOut = async() => {
        try {
            ...
            //another method toast error(error.data.message), but it throws error
            toast.error("execution reverted: You have a pending balance. or You cannot rent at this time.", {
                position: toast.POSITION.TOP_RIGHT
              });
        }
